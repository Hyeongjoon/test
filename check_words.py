...
test = openpyxl.load_workbook(filename='words_data.xlsx')
sheet = test.worksheets[0]
exception = openpyxl.load_workbook(filename='words_exception.xlsx')
...
for i in range(2, max_data_row):
    ...
    for j in range(0,len(arr)):
        resultSheet.cell(row=i,column=1).value= str(resultSheet.cell(row=i,column=1).value)+ " " + str(arr[j])
